...
function scheduleMetaText(schedule) {
  const parts = [`${schedule.team} ${schedule.season} ${schedule.sport}`];
  if (schedule.source_checked) {
    parts.push(`source checked ${schedule.source_checked}`);
  }
  if (schedule.sample) {
...
async function init() {
  ...
  els.scheduleMeta.textContent = scheduleMetaText(state.schedule);
  renderGames();
  if (state.games.length > 0) {
    await selectGame(state.games[0]);
  }
}
